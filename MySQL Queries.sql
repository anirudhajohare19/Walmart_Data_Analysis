...
# Checking Payment Methods and count of payments Through each method
SELECT 
...
SELECT 
    payment_method, COUNT(*) AS no_payments, SUM(quantity)
FROM
    walmart_sales
GROUP BY payment_method;
...
FROM (
    SELECT 
        ...
        RANK() OVER(PARTITION BY branch ORDER BY COUNT(*) DESC) AS rannk
    FROM walmart_sales
